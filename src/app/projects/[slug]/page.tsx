...
export default function ProjectDetail({ params }: ProjectDetailProps) {
  ...
  if (!slug)
    return (
      <div className="min-h-screen flex items-center justify-center text-white bg-[#000000]">
        Loading...
      </div>
    );
  ...
  return (
    <section className="min-h-screen bg-[#000000] text-white pt-24 lg:pt-32 pb-[6rem] px-4 sm:px-12 py-4 sm:py-12 font-sans selection:bg-white/20 ">
      ...
      <div className="flex flex-col lg:flex-row gap-10 xl:gap-16 mb-6 lg:mb-8 pb-8 border-b border-white/20">
          ...
          {project.images && project.images.length > 0 && (
            <div className="w-full overflow-hidden shadow-[0_20px_50px_rgba(255,255,255,0.02)] border border-white/10 bg-[#050505] relative">
              <div className="relative group/slider">
                ...
                <div
                  className={`swiper-prev-${project.slug} absolute inset-y-0 left-0 w-12 sm:w-24 bg-gradient-to-r from-black/60 to-transparent z-10 flex items-center justify-start pl-2 sm:pl-4 cursor-pointer text-white`}
                >
                  <svg
                    className="w-6 h-6 sm:w-12 sm:h-12 opacity-70 hover:opacity-100 transition-opacity"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                ...
                <div
                  className={`swiper-next-${project.slug} absolute inset-y-0 right-0 w-12 sm:w-24 bg-gradient-to-l from-black/60 to-transparent z-10 flex items-center justify-end pr-2 sm:pr-4 cursor-pointer text-white`}
                >
                  <svg
                    className="w-6 h-6 sm:w-12 sm:h-12 opacity-70 hover:opacity-100 transition-opacity"
                    fill="none"
        ...
        <div className="w-full lg:w-[450px] shrink-0 flex flex-col gap-6 lg:gap-10">
          {/* Header Section */}
      ...
      {/* Content Section */}
      <div className="w-full mx-auto flex flex-col gap-8 md:gap-12">
        {/* About Section */}
        <div className="flex flex-col gap-6">
          <p className="text-base sm:text-lg lg:text-xl text-zinc-200 leading-[1.8] font-medium">
            {project.about}
          </p>
        </div>
        ...
        {project.features && project.features.length > 0 && (
          <div className="flex flex-col gap-6 sm:gap-8 bg-[#050505] p-4 sm:p-10 border border-white/10 shadow-sm">
            ...
            <ul className="flex flex-col gap-5 sm:gap-6">
              {project.features.map((feature, idx) => (
                <li key={idx} className="flex items-start gap-3 sm:gap-5">
                  <div className="mt-1 flex items-center justify-center w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-white/10 text-white shrink-0 border border-white/5">
                    <svg
                  ...
                  <span className="text-sm sm:text-base md:text-lg text-zinc-200 leading-[1.7] font-medium">
                    {feature}
                  </span>
                </li>
              ))}
